...
_order_handler.post = (data, callback) => {
    ...
    if (key && number && exp_month && exp_year && cvc && name) {
        // Create card object.
        var cardDetails = {
            'number': number,
            'exp_month': exp_month,
        ...
        _token_handler.verifyToken(key, (status, tokenDetails) => {
            if (status && tokenDetails.phone > 0) {
                ...
                file_exp.read('users', tokenDetails.phone, (err, userDetails) => {
                    if (!err && userDetails && userDetails.cart !== undefined) {
                        // Get order details.
                                    ...
                                    if (!err) {
                                        ...
                                        helpers.stripeCreateCardToken(cardDetails, (err, tokenObj) => {
                                            if (!err && tokenObj) {
                                                ...
                                                helpers.stripeCreateCharge(tokenObj.id, orderDetails.order.total, desc, (err, chargeObj) => {
                                                    if (!err && chargeObj && chargeObj.paid) {
                                                                    ...
                                                                    if (!err) {
                                                                        ...
                                                                        file_exp.read('invoice', orderDetails.orderId, 'html', (err, html) => {
                                                                            if (!err) {
                                                                                ...
                                                                                callback(401, { 'Error': 'Failed to get order details.' });
                                                                            }
                                                                        });
                                                                    } else {
                                                        ...
                                                        callback(402, { 'Error': 'Invalid card details.' });
                                                    }
                                                });
                                            } else {
                                                callback(402, { 'Error': 'Payment token request failed.' });
                                            }
                                        });
                                    } else {
                                        callback(402, { 'Error': 'Failed to generate invoice' });
                                    }
                                });
                            } else {
...
_order_handler.generateInvoice = (orderDetails, callback) => {
    ...
    file_exp.read('invoice', 'invoice_frame', 'html', (err, html) => {
        if (!err && html.length > 0) {
            ...
            // Create invoice file.
            file_exp.create('invoice', orderDetails.orderId, 'html', invoiceHtmlData, (err) => {
                if (!err) {
                    callback(false);
                } else {
                    callback(true);
