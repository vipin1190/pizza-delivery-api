 ...
 * @param {number} phone a 10 digit long valid phone number.
 * @param {callback} callback returns whether key is valid or not.
 */
_token_handler.verifyToken = (key, phone, callback) => {
  file_exp.read('tokens', key, (err, tokenDetails) => {
    if (!err && tokenDetails !== undefined && tokenDetails.phone == phone && tokenDetails.expires > Date.now()) {
      callback(true);
    } else {
      callback(false);
    }
