...
_users_handler.put = (data, callback) => {
  ...
  if (phone && key) {
    _token_handler.verifyToken(key, phone, (status) => {
      if (status) {
        file_exp.read('users', phone, (err, UserProfileObject) => {
          if (!err && UserProfileObject) {
            var firstName = (typeof (data.payload.firstName) == 'string' && data.payload.firstName.trim().length > 0) ? data.payload.firstName.trim() : false;
            var lastName = (typeof (data.payload.lastName) == 'string' && data.payload.lastName.trim().length > 0) ? data.payload.lastName.trim() : false;
            var address = (typeof (data.payload.address) == 'string' && data.payload.address.trim().length > 0) ? data.payload.address.trim() : false;
            var password = (typeof (data.payload.password) == 'string' && data.payload.password.trim().length > 0) ? data.payload.password.trim() : false;
            ...
            if (firstName || lastName || address || password) {
              ...
              // Update user profile.
              file_exp.update('users', phone, UserProfileObject, (err) => {
                if (!err) {
                  callback(200);
                } else {
            ...
            callback(404, { 'Error': 'User not available.' });
          }
        });
      } else {
...
_users_handler.get = (data, callback) => {
  ...
  if (phone && key) {
    _token_handler.verifyToken(key, phone, (status) => {
      if (status) {
        file_exp.read('users', phone, (err, data) => {
          if (!err && data) {
            delete data.hashedPasscode;
            callback(200, data);
          } else {
...
_users_handler.delete = (data, callback) => {
  ...
  if (phone && key) {
    _token_handler.verifyToken(key, phone, (status) => {
      if (status) {
        file_exp.exists('users', phone, (status) => {
          if (status) {
            file_exp.delete('users', phone, (err) => {
              if (!err) {
                callback(200);
              } else {
